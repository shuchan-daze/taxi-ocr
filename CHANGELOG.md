...
Shuchan の哲学「日報は罫線で区切られた 2D グリッド (= データシート)」を
OCR プロンプトと内部処理に反映する第一歩.

**1. OCR プロンプト改訂 (Phase 2)**

冒頭に「最重要」セクションを新設:
- 日報は文字列の流れではなく、罫線で区切られた絶対座標のグリッド
- 行数は紙の罫線の数で機械的に決まる、AI が判断する権限は無い
- 罫線で区切られた区画は 1 つでも見逃さず出力
- 数字が薄い・小さい・滲んでも罫線内に書かれていればその行は値ありとして読む
- 行の見落としは致命的な失敗

**2. paper_row フィールド追加 (Phase 3)**

- `NIPPOU_COLUMNS` に `paper_row` を追加 (= 紙の罫線で区切られた区画の上から番号)
- `interpret_raw_rows` で paper_row を ride にそのまま伝播
- 新関数 `validate_paper_row_continuity`: paper_row が連番か検証、欠番を検出

これによって AI が紙の行を見落とした場合、Python 側で paper_row 不連続として
検出可能になる. 「行が消える」が原理的に発生しないグリッドの土台.

### Background

Shuchan の指摘 (2026-05-17):
- 380 円の障害者割引行が OCR で消えた事例
- 「データシートであるから行や列が勝手に消えることは無いんだよね」
- 文字列処理 (上から順に並べるだけ) では行の漏れを構造的に防げない
- 縦横の行と列によって空間を認識してる状態でなければいけない

### Tests

- 全 109 件 pass (新規 4 件):
  - paper_row が ride に伝播することを保証
  - 連番検証 (連続 / 欠番 / 互換性)

### 次のフェーズ予定

- Phase 4: build_report で paper_row 不連続を警告表示
- Phase 5: 確認 UI を統一フロー化 (mismatch + missing + fallback)
- Phase 6: 最終テスト + 実機検証
...
### Fixed (障害者割引 2 件以上でアプリがクラッシュする問題)

実例: ¥270 と ¥380 の障害者割引が両方とも `no='20+'` を持ったため、
`st.radio` の widget key `discount_status_20+` が重複して
`StreamlitDuplicateElementKey` でアプリ全体がクラッシュしてた.

修正: `DiscountEmitter` のラベルを貸切と同じ通し番号形式 (`障1`/`障2`/`障3`...) に変更.
no がユニークになり、widget key 衝突が原理的に発生しなくなる.

### Tests

- 全 105 件 pass (新規 1 件: 複数件の障割で no が一意 + 衝突しないことを保証)
...
実例で発覚した問題: AI が障割行を ¥300 と読んだが、実際は ¥380。手書き
数字 (特に 0/8、3/8 等) は誤読しやすい。ユーザーが打ち直して確定する
仕組みを追加。

- `apply_user_choices` を拡張: `discount_amount_{no}` キーで mi を上書き
- 新関数 `render_discount_confirmations`: discount 行ごとに `st.number_input`
  で金額入力 UI を表示. AI 読み取り値を初期値、10 円刻みで調整可能.
- 集計はリアルタイム再計算

### Tests

- 全 98 件 pass (新規 2 件: discount override の単体テスト)

### 設計思想

「正確さ第一、コスト削減は二の次。間違ったら意味がない」
([[feedback-accuracy-first]]) に沿う. 障害者割引のような税務に直結する
数字は、AI 誤読でズレるとそのまま申告ミスになるため、ユーザー側で
最終確定できる UI が必須.
...
### Added (状況証拠ベースの貸切自動判定 + 未記載行の選択肢 UI)

**1. 貸切の状況証拠化 (D)**

memo に「貸切」と書いてなくても、メーター明細にアラインできなかった
「孤立 ride」を自動的に貸切候補として Layer 3 に昇格させる。

- `build_report` で `_align_rides_to_meter` 後に孤立 ride を検出
- 金額 ¥1,000 以上を charter として promote (¥1,000 未満は OCR ゴミ判定)
- memo に `(自動判定: 貸切)` マーカーを追記、`needs_review=True` で確認促進
- 既に明示的に `case='charter'` の ride は二重マーカー回避

これで Shuchan が「頭にも貸切と書く」「摘要に貸切と書く」を忘れても、
アプリ側が「メーター明細に対応しない金額あり = 貸切の可能性」と
自動推論する。状況証拠から AI が判断する設計。

**2. 未記載行 (missing_nippou) の現収/未収選択肢 UI (E)**

メーター明細にあって紙日報に書き忘れた行は、これまで「現収と仮定」で
集計に貢献させてた。今回からユーザーがラジオで 現収/未収 を選び直せる。

- 新関数 `apply_user_choices(rows, choices=None)`: 選択値で gen/mi を入れ替え
- 新関数 `render_missing_choices(rows)`: 行ごとにラジオ UI を描画
- session_state 経由で選択値を保持、選択変更で自動再計算
- ラベル例: `No.3 (14:36) ¥3,200` + 現収/未収 ラジオ

### Tests

- 全 96 件 pass
- 新規: `TestOrphanCharterPromotion` (3 件) — 孤立 ride の貸切昇格
- 新規: `test_apply_user_choices_*` (3 件) — 選択肢 UI のロジック

### 設計図の完成度

3 レイヤー設計図 (v1.25.01〜03) + イレギュラー検出の文脈推論 (D) +
選択肢ポップアップ UI (E) で、Shuchan のメンタルモデルの主要部分が
コードに反映された.
