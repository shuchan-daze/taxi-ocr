...
判定方法：
- 日報の現収/未収欄の金額（＝客が支払った額）と、メーター明細の金額が異なる
- 日報側に「+〇〇」「超過」「消し忘れ」等の追記がある（または通常欄とは別に小さく書かれた金額がある）
- 差額（メーター明細 − 日報の現収/未収）= メーター超過分

出力方法：
- 客分行：日報の金額を、日報で数字が書かれている側（現収 or 未収）に記録。摘要は日報の記載通り（例「現金」「Visa」など）。状態は「ok」。
- 超過分行：差額を必ず「現収」に記録。摘要は「メーター超過」。状態は「mismatch」。
- 超過分が必ず「現収」になる理由：差額は乗務員が現金で会社に納金するため。

【メーター超過のパターン例】
パターンA：客が現金で支払った場合
日報：「現収 1,600円」と「+100円（メーター超過）」、メーター明細：1,700円
→ 2行に分ける：
   行1：現収 1,600 摘要「現金」 状態 ok
   行2：現収 100 摘要「メーター超過」 状態 mismatch

パターンB：客がVisa等のカードで支払った場合
日報：「未収 1,600円 Visa」と「+100円（メーター超過）」、メーター明細：1,700円
→ 2行に分ける：
