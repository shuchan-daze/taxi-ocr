...
if file1 and file2:
    if st.button('日報を完成させる', use_container_width=True, type='primary'):
        with st.spinner('照合中...'):
            ...
            prompt = '2枚の画像のうち、1枚は手書きのタクシー日報、もう1枚はメーター明細書です。どちらがどちらかを自動判断してください。現収か未収かの判定は手書き日報の現収欄・未収欄に数字が書いてある方に従う。金額はメーター明細の値を正として使用する。摘要は手書き日報の記載に従う。日報をマークダウンテーブルで出力。列：No/人数/降車時刻/現収/未収/摘要。最後に現収合計・未収合計・総営収・消費税（総営収を11で割り10円単位四捨五入）・税抜運収（総営収マイナス消費税）を表示。'
