...
if len(imgs) == 2:
    if st.button('🔍 日報を完成させる', use_container_width=True, type='primary'):
        ...
        try:
            ...
            st.stop()
        try:
                prompt = """【最重要ルール】金額の基準はメーター明細書（2枚目の画像）。ただし「メーター超過（消し忘れ）」のケースでは、客の支払額（日報）とメーター明細の金額の両方を使い、1行を2行に分けて記録する。

手順：
1. メーター明細書（2枚目）の各行の時刻と金額を全て読み取る
2. 日報（1枚目）の各行を読み取り、現収欄・未収欄のどちらに、いくらの数字が書かれているか確認する
3. 各行について、日報の金額（客の支払額）とメーター明細の金額を比較する：
   (a) 一致する場合（通常）：メーター明細の金額を、日報で数字が書かれている側（現収 または 未収）に1行で記録する。摘要は日報の記載をそのまま使う。
   (b) 異なる場合（メーター超過＝消し忘れ。メーター額のほうが多い）：1行を以下の2行に分けて出力する。
       ・客分：日報の金額を、日報で数字が書かれている側（現収 or 未収）に記録。摘要は日報の記載通り（例「現金」「Visa」など）。
       ・超過分：差額（メーター明細の金額 − 日報の金額）を必ず「現収」側に記録。摘要は「メーター超過」と記載。
         ※超過分が必ず「現収」になる理由：差額は乗務員が現金で会社に納金するため。
4. 件数（## 集計の件数）は乗車件数で数える。メーター超過で2行に分けた場合も1件として数える。

【メーター超過のパターン例】
- 客が現金で1,600円支払い、メーター明細は1,700円の場合 → 2行に分ける
   1行目：現収 1,600 摘要「現金」
   2行目：現収 100 摘要「メーター超過」
- 客がVisaで1,600円支払い、メーター明細は1,700円の場合 → 2行に分ける
   1行目：未収 1,600 摘要「Visa」
   2行目：現収 100 摘要「メーター超過」

【絶対禁止】
- メーター明細に存在しない金額を捏造しないこと
- 通常行（メーター超過なし）で日報の金額を優先しないこと
- メーター超過の差額を未収側に記録しないこと（必ず現収）
...
with st.expander('？ このアプリについて・使い方'):
    st.markdown('''
### なぜこのアプリが生まれたか
タクシー乗務員は毎日の業務後、日報入力に **約30分** を費やしています。
年間にすると **180時間以上** ── まるまる一週間以上を、書き写し作業に取られているのです。
「この時間、もっと休んだり家族と過ごしたりできるはずだ」── そう思ったのが出発点でした。
...
これまで人間がやっていた頭脳労働を、AIに肩代わりさせます。

### 何がすごいか
このアプリは従来の「OCR（文字を読み取るだけ）」ではありません。
AIが画像そのものを **「理解」** して、文脈や意味を読み取って処理します。
ほんの数年前までは研究段階だった技術が、いま現場のタクシードライバーの手元で動いています。

### プライバシーへの配慮
- このアプリのサーバーには **一切保存されません**
- 画像はAnthropic社（Claudeの開発元）に一時的に送信されます
- 送信されたデータは **AIの学習には使われず**、**30日以内に自動削除** されます
- 個人情報や業務情報が外部に残ることはありません

### 使い方
1. 写真を **2枚**（日報・営業明細書）アップロード
2. ボタンを押す
3. 完成
