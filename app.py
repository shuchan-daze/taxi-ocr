...
if len(imgs) == 2:
    if st.button('🔍 日報を完成させる', use_container_width=True, type='primary'):
        ...
        try:
            ...
            st.stop()
        try:
                prompt = """あなたはタクシー日報処理の専門家です。

【絶対遵守ルール - これに違反したら全て無効】
1. 金額は必ずメーター明細書（2枚目の画像）から読み取る。
2. 日報（1枚目）の金額は参考にすらしない、無視する。例外は以下の2つのみ：
   (a) メーター超過（消し忘れ）の「+〇〇」追記 → 後述「【例外：メーター超過の処理】」
   (b) 貸切（メーター不使用） → 後述「【例外：貸切の処理】」
3. 出力する金額は、メーター明細書に記載されている数字そのもの（メーター超過時は分割、貸切時は日報金額を使う）。

【金額読み取りの厳密ルール】※精度最優先。直近の誤読例：1,800→1,200／1,100→5,100／3,300→3,200／1,500→1,400。
...
3. 形が紛らわしい数字ペアに最大限注意：「3 と 2」「8 と 2」「8 と 3」「5 と 4」「6 と 0」「7 と 1」「9 と 4」「1 と 5」（1,100→5,100の誤読防止）。少しでも迷ったら拡大して再確認する。
4. 不確実な場合でも、前後の金額や時刻、合計値などの「文脈」から数字を推測してはいけない。あくまで画像上の文字を読む。
5. メーター明細書の数字を「絶対の真実」として扱い、日報の手書き数字とは独立して読み取る。日報の数字に引きずられないこと。

【処理手順】

Step 1: メーター明細書の各行を順番に読み取る。
- 「No. 時刻 ¥金額」の形式で全行リストアップする。例：「1. 10:32 ¥4,100」「2. 10:50 ¥1,000」...
- この時点で、日報を見てはいけない（日報の数字に引きずられないため）。
- 各金額は桁ごとに慎重に読む。

Step 2: メーター明細書から読み取った金額のリストを内部で書き出す。

Step 3: 日報を見て、各行について以下のみ確認する。金額は見ない（後述の例外2種類を除く）。
- 現収か未収か（どちらの欄に書いてあるか）だけ判定。
- 摘要（Visa、Uber、交通系、現金、障割、貸切 等）を読み取る。
- 例外1（メーター超過）: 通常の現収/未収欄とは別に「+〇〇」「メーター超過」「消し忘れ」「超過」等の追記がある行のみ、その「+〇〇」の値を超過分として読み取る。
- 例外2（貸切）: 日報に「貸切」「貸し切り」「チャーター」等の記載がある行のみ、その行の現収/未収欄の金額を読み取る（メーターを使わない料金体系のため）。

Step 4: メーター明細の金額を、日報の判定（現収/未収）に従って配置する。
- 摘要は日報から読み取った値を使う。
- 例外1（メーター超過の追記がある場合）: 後述「【例外：メーター超過の処理】」に従い、その行を2行に分割する。
- 例外2（貸切の記載がある場合）: 後述「【例外：貸切の処理】」に従い、メーター明細ではなく日報の金額を採用する。
- 注意（障害者割引）: 日報に「障割」の記載がある行は、メーター明細にすでに割引後の金額が表示されている。よって通常処理（メーター明細の金額をそのまま採用）。摘要を「障割」と記載するだけ。

【出力前チェック - 必須】
出力する各行の金額が、Step 1で読み取ったメーター明細の数字と完全一致していることを確認する。
- 1円でも違う場合は、Step 1からやり直す。
- 特に紛らわしい数字（3/2、8/2、5/4、1/5、6/0、9/4）について、桁を再確認する。
- メーター超過で2行に分割した場合は、客分 + 超過分 = メーター明細の金額が成り立つことを確認する。成り立たなければ「+〇〇」の読み違い、または客分の引き算ミスを疑う。

【例外：メーター超過（消し忘れ）の処理】
日報に「+〇〇円」「メーター超過」「消し忘れ」「超過」等の追記がある行に限り、1行を2行に分割する。これが Step 3 「金額は見ない」の唯一の例外。
...
★計算手順（必ずこの順）
E1: 日報の「+〇〇」を読み取り、その値を「超過分」とする（メーター明細からは読み取れない。日報の追記からのみ読み取れる）。
E2: 客分の金額 = メーター明細の金額 − 超過分（必ず引き算で求める）。
E3: 客分行と超過分行の2行に分けて出力する。

★出力方法
- 客分行：客分の金額（= メーター明細 − 超過分）を、日報で数字が書かれている側（現収 or 未収）に記録。摘要は日報の記載通り（例「現金」「Visa」など）。状態は「ok」。
- 超過分行：超過分（日報の「+〇〇」の値）を必ず「現収」に記録。摘要は「メーター超過」。状態は「mismatch」。
- 超過分が必ず「現収」になる理由：差額は乗務員が現金で会社に納金するため。
- 検証：客分 + 超過分 = メーター明細の金額（必ず一致。一致しなければ「+〇〇」の読み違いを疑い、桁を再確認）。

★パターン例
パターンA：客が現金で支払った場合
日報：現収欄に「1,600」、別途「+100（メーター超過）」、メーター明細「¥1,700」
処理：超過分 = 100、客分 = 1,700 − 100 = 1,600
→ 行1：現収 1,600 摘要「現金」 状態 ok
→ 行2：現収 100 摘要「メーター超過」 状態 mismatch

パターンB：客がVisa等のカードで支払った場合
日報：未収欄に「1,600 Visa」、別途「+100（メーター超過）」、メーター明細「¥1,700」
処理：超過分 = 100、客分 = 1,700 − 100 = 1,600
→ 行1：未収 1,600 摘要「Visa」 状態 ok
→ 行2：現収 100 摘要「メーター超過」 状態 mismatch

パターンC：超過分が異なる金額（+200のケース）
日報：未収欄に「2,800 Visa」、別途「+200（メーター超過）」、メーター明細「¥3,000」
処理：超過分 = 200、客分 = 3,000 − 200 = 2,800
→ 行1：未収 2,800 摘要「Visa」 状態 ok
→ 行2：現収 200 摘要「メーター超過」 状態 mismatch

【例外：貸切（チャーター）の処理】
日報に「貸切」「貸し切り」「チャーター」等の記載がある行は、メーター明細ではなく日報の金額を採用する。これが Step 3 「金額は見ない」のもう1つの例外。

理由：貸切（チャーター）はメーターを使わない事前合意の料金体系のため、メーター明細書には貸切料金が反映されていない（金額が0円・空欄・別額になる場合がある）。日報に書かれた金額が正しい料金。

判定方法：
- 日報の摘要欄や行の備考に「貸切」「貸し切り」「チャーター」等の記載がある
- 単独の判断基準。メーター明細の金額が異なるからといって貸切と判定しない（必ず日報側の明示記載を確認する）

出力方法：
- 金額：日報の現収/未収欄に書かれている金額をそのまま使う（メーター明細の金額は使わない）
- 配置：日報で数字が書かれている側（現収 or 未収）に記録
- 摘要：「貸切」と記載
- 状態：「ok」（メーター明細とは別系統の正規料金のため警告は不要）

パターンD：貸切（現金で受領）
日報：現収欄に「12,000」、摘要「貸切」、メーター明細：該当時刻に対応する金額が表示されない、または異なる
→ 行1：現収 12,000 摘要「貸切」 状態 ok

パターンE：貸切（後日請求＝未収）
日報：未収欄に「15,000」、摘要「貸切 △△商事」、メーター明細：該当行が無いか、異なる金額
→ 行1：未収 15,000 摘要「貸切 △△商事」 状態 ok

【参考：障害者割引（障割）の処理】
日報に「障割」と記載がある行は、メーター明細にすでに割引後の金額が表示されているため、通常処理（メーター明細の金額をそのまま採用）でよい。例外ではない。

出力方法：
- 金額：メーター明細の金額をそのまま採用（割引後の金額がそのまま明細に出る）
- 配置：日報で数字が書かれている側（現収 or 未収）に記録
- 摘要：「障割」と記載（その他の摘要が併記されていれば併せて記載：例「障割 現金」）
- 状態：「ok」

パターンF：障害者割引
日報：現収欄に「1,440」、摘要「障割 現金」、メーター明細：「¥1,440」（割引後）
→ 行1：現収 1,440 摘要「障割 現金」 状態 ok

【件数】
件数（## 集計の件数）はメーター明細の行数（乗車件数）で数える。メーター超過で2行に分けた場合も1件として数える。

【絶対禁止】
- メーター明細に存在しない金額を出力すること（貸切時を除く）
- 日報の現収/未収欄の手書き金額を読み、それを出力すること（例外は「メーター超過の+〇〇」と「貸切」のみ）
- メーター明細の金額を勝手に変更（四捨五入、丸め、桁の入替）すること
- メーター超過の追記があるのに見落として1行だけにすること
- 超過の追記がないのに勝手に分割して超過行を作ること
- メーター超過の差額を未収側に記録すること（必ず現収）
- 出力金額の合計（分割行も含む）がメーター明細と一致しないこと（貸切行を除く）
- 日報に「貸切」と書かれていないのに勝手に貸切と判定すること
- 障割の行にも勝手に貸切処理や超過分割を適用すること（障割は通常処理）
