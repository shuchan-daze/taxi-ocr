...
if len(imgs) == 2:
    if st.button('🔍 日報を完成させる', use_container_width=True, type='primary'):
        ...
        try:
            ...
            st.stop()
        try:
                prompt = """★★★絶対ルール★★★（最優先・例外なし）
日報とメーター明細で金額が異なる場合、メーター明細の金額を必ず採用すること。
日報の金額は無視すること。例外なし。

具体例：
- 日報に「1,200円」と書かれているが、メーター明細書に「¥1,800円」とある場合
  → 出力すべき金額は 1,800円（メーター明細を採用）
  → 状態列は「mismatch」（日報の記載と異なるため警告として記録）
- 日報に「3,200円」と書かれているが、メーター明細書に「¥3,300円」とある場合
  → 出力すべき金額は 3,300円
  → 状態列は「mismatch」

【出力前のセルフチェック・必須】
出力を確定する前に、各行について以下を必ず行うこと：
1. その行の金額が、メーター明細書のどの行・どの金額と対応しているか確認する
2. 出力する金額がメーター明細書の値と完全に一致しているか最終確認する
3. 日報の手書き金額に引きずられていないか、もう一度読み比べる
セルフチェックで日報の値を出力していると気付いたら、必ずメーター明細の値に置き換えてから出力すること。
...
2. 日報（1枚目）の各行を読み取り、現収欄・未収欄のどちらに数字が書かれているか確認する（金額そのものは「絶対ルール」によりメーター明細を採用するため、日報の金額は参考扱い）
3. 各行について、メーター明細の金額を、日報で数字が書かれている側（現収 または 未収）に1行で記録する。摘要は日報の記載をそのまま使う。
4. 状態列：メーター明細の金額と日報の手書き金額が一致していれば「ok」、異なっていれば「mismatch」（メーター値を採用したことのユーザー向け警告）。
5. 件数（## 集計の件数）はメーター明細の行数（乗車件数）で数える。

【絶対禁止】
- メーター明細に存在しない金額を出力しないこと
- 日報の手書き金額を採用しないこと（必ずメーター明細の値を採用）
- 不一致行を「ok」とすること（必ず「mismatch」）
...
【状態列の判定ルール】
- 「ok」: メーター明細の金額と日報の手書き金額が一致している
- 「mismatch」: メーター明細の金額を採用したが、日報の手書き金額と異なる（要確認の警告）
