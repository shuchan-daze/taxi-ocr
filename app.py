...
2. 日報（1枚目）の金額は参考にすらしない、無視する（後述「メーター超過の例外」のみ唯一の例外）。
3. 出力する金額は、メーター明細書に記載されている数字そのもの（メーター超過時は、メーター額を客分と超過分に分割して合計を一致させる）。
...
Step 3: 日報を見て、各行について以下のみ確認する。金額は見ない（後述の例外を除く）。
- 現収か未収か（どちらの欄に書いてあるか）だけ判定。
- 摘要（Visa、Uber、交通系、現金、障割 等）を読み取る。
- 例外: 通常の現収/未収欄とは別に「+〇〇」「メーター超過」「消し忘れ」「超過」等の追記がある行のみ、その「+〇〇」の値を超過分として読み取る。それ以外の金額は一切見ない。

Step 4: メーター明細の金額を、日報の判定（現収/未収）に従って配置する。
- 摘要は日報から読み取った値を使う。
- 例外（メーター超過の追記がある場合）: 後述「【例外：メーター超過の処理】」に従い、その行を2行に分割する。
...
【絶対禁止】
- メーター明細に存在しない金額を出力すること
- 日報の現収/未収欄の手書き金額を読み、それを出力すること（メーター超過の「+〇〇」追記のみ唯一の例外）
...
【状態列の判定ルール】
- 「ok」: 通常行（メーター明細の値を採用）、またはメーター超過分割時の客分行
