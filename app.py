...
if len(imgs) == 2:
    if st.button('🔍 日報を完成させる', use_container_width=True, type='primary'):
        ...
        try:
                prompt = """★★★絶対ルール★★★（最優先）
出力する全ての金額の合計（または1行を分割した場合は分割行の合計）は、必ずメーター明細書の金額と一致させること。
日報とメーター明細で金額が異なる場合の処理は、以下の2系統に分岐する：

(1) 日報に「+〇〇円」「メーター超過」「消し忘れ」「超過」等の追記がある場合
  → 1行を2行に分けて出力する（後述「【メーター超過（消し忘れ）の処理】」を厳守）。
  → 客分行は日報の金額のまま、超過分行は差額を「現収」に記録する。
  → 客分 + 超過分 ＝ メーター明細の金額（合計が一致）。

(2) 上記のような追記がなく、単に日報とメーター明細の金額が違うだけの場合
  → メーター明細の金額を採用し、日報の値は無視する。例外なし。
  → 状態列は「mismatch」（要確認の警告として記録）。

具体例：
- 日報に「1,200円」のみ（超過の追記なし）／メーター明細に「¥1,800円」
  → 出力 1,800円（メーター値を採用）、状態 mismatch
- 日報に「3,200円」のみ（超過の追記なし）／メーター明細に「¥3,300円」
  → 出力 3,300円、状態 mismatch
- 日報に「未収 1,600円 Visa」と「+100円 メーター超過」／メーター明細に「¥1,700円」
  → 2行に分ける（パターンB参照）

【出力前のセルフチェック・必須】
出力を確定する前に、各行（および分割行）について以下を必ず行うこと：
1. メーター明細書のどの行・どの金額と対応しているか確認する
2. 出力金額（分割した場合は2行の合計）がメーター明細の値と完全一致しているか最終確認する
3. 日報の手書きに「+」「超過」「消し忘れ」等の追記を見落としていないか再確認する
4. 日報の手書き金額に引きずられていないか、もう一度読み比べる
セルフチェックで誤りに気付いたら、必ずメーター明細の値に修正してから出力すること。

【金額読み取りの厳密ルール】※精度最優先。直近の誤読例：3,300→3,200／1,800→1,200／1,500→1,400。
...
3. 形が紛らわしい数字ペアに最大限注意：「3 と 2」「8 と 2」「8 と 3」「5 と 4」「6 と 0」「7 と 1」「9 と 4」。少しでも迷ったら拡大して再確認する。
4. 不確実な場合でも、前後の金額や時刻、合計値などの「文脈」から数字を推測してはいけない。あくまで画像上の文字を読む。
5. メーター明細書の数字を「絶対の真実」として扱い、日報の手書き数字とは独立して読み取る。日報の数字に引きずられないこと。

【メーター超過（消し忘れ）の処理 - 最重要】
日報に「+〇〇円」「メーター超過」「消し忘れ」「超過」等の追記がある場合、その行は必ず2行に分けて出力する。

★「+」記号の認識（最重要・誤読厳禁）
- 「+〇〇」の先頭にある「+」記号を絶対に見落とさないこと。
- 「+100」を「1,100」と誤読しない。「+200」を「1,200」と誤読しない。「+〇〇」の「+」は加算記号であり、後ろの数字は超過分（自己負担額）。
- 「+」が判読しづらくても、通常の現収/未収欄とは別に小さく書かれた数字はメーター超過分の候補として扱う。
- 超過分は固定値ではなく、ケースごとに毎回異なる金額（+50、+100、+200、+300 など）。日報の「+〇〇」をそのまま読み取ること。

★超過分・客分の計算手順（必ずこの順）
Step 1: 日報の「+〇〇」を読み取り、その値を「超過分」とする（メーター明細からは読み取れない。日報からのみ読み取れる）。
Step 2: 客分の金額 = メーター明細の金額 − 超過分（必ず引き算で求める）。日報の「現収/未収」欄に書かれている数字と一致するはず（一致しない場合は「+〇〇」の読み違いを疑い、桁を再確認）。
Step 3: 客分行＋超過分行の2行に分けて出力する。

判定方法：
- 日報側に「+〇〇」「超過」「消し忘れ」等の追記がある
- メーター明細の金額 ＞ 日報の現収/未収欄の金額
...
- 検証：客分 + 超過分 = メーター明細の金額（必ず一致。一致しなければどこかで読み違いがあるので再確認）。

【メーター超過のパターン例】
パターンA：客が現金で支払った場合
日報の記載：現収欄に「1,600」、別途「+100（メーター超過）」
メーター明細：「¥1,700」
処理：
- 超過分 = 100（日報の「+100」を読む。「1,100」と誤読しない）
- 客分 = 1,700 − 100 = 1,600
→ 2行に分ける：
...
日報の記載：未収欄に「1,600 Visa」、別途「+100（メーター超過）」
メーター明細：「¥1,700」
処理：
- 超過分 = 100（日報の「+100」を読む）
- 客分 = 1,700 − 100 = 1,600
...
日報の記載：未収欄に「2,800 Visa」、別途「+200（メーター超過）」
メーター明細：「¥3,000」
処理：
- 超過分 = 200（日報の「+200」を読む。「1,200」「2,200」等と誤読しない）
- 客分 = 3,000 − 200 = 2,800
→ 2行に分ける：
...
2. 日報（1枚目）の各行を読み取り、以下を確認する：
   (a) 現収/未収欄の金額
   (b) 摘要欄の記載
   (c) 「+〇〇」「超過」「消し忘れ」等の追記の有無
3. 各行について、以下のように処理する：
   (a) 日報に超過の追記あり → 「メーター超過の処理」に従い2行に分割。客分=ok、超過分=mismatch。
   (b) 追記なしで、日報とメーター明細の金額が一致 → メーター明細の金額を、日報で数字が書かれている側に1行で記録。状態 ok。
   (c) 追記なしで、日報とメーター明細の金額が異なる → メーター明細の値を採用、状態 mismatch。
4. 件数（## 集計の件数）はメーター明細の行数（乗車件数）で数える。メーター超過で2行に分けた場合も1件として数える。

【絶対禁止】
- メーター明細に存在しない金額を出力しないこと
- 出力金額の合計（分割行も含む）がメーター明細と一致しないこと
- 日報に超過の追記があるのに見落として1行だけにすること
- 超過の追記がないのに勝手に分割して超過行を作ること
- メーター超過の差額を未収側に記録すること（必ず現収）
- 不一致行（mismatch条件）を「ok」とすること
...
【状態列の判定ルール】
- 「ok」: メーター明細の金額と日報の手書き金額が一致している、またはメーター超過分割時の客分行（金額が日報通り）
- 「mismatch」: メーター明細の値を採用したが日報と異なる、またはメーター超過分割時の超過分行（摘要「メーター超過」）
